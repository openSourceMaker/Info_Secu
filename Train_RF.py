...
def train_random_forest(train_x, train_y):
    ...
    predicted = rf.predict(x_train_input)
...
def test_random_forest(test_x, test_y):

    x_test_input = test_x
    y_test_input = test_y

    # partially testing, 10% of the dataset
    predicted = rf.predict(x_test_input)
    return_predicted = predicted
    return_label = y_test_input
    ...
    return return_label, return_predicted, acc
...
def getStatistics(ms, y_label, y_predicted, acc):
    ...
    with open("./_result_RandomForest_inSequence/result_random_forest_inSequence.csv", "w") as csvfile:
...
def draw_confusion_matrix(y_label, y_predicted, acc):
    ...
    plt.figure()
    plot_confusion_matrix(cnf_matrix, classes = "01", title = "Confusion Matrix for Random Forest")
    plt.savefig("./_result_RandomForest_inSequence/cm_random_forest_inSequence.png")
    plt.show()
...
def draw_roc_curve(label, predicted):
    ...
    plt.savefig("./_result_RandomForest_inSequence/roc_random_forest_inSequence.png")
    plt.show()
...
def saveResults(y_label, y_predicted):
    ...
    with open("./ResultsModelsLabels/RandomForest_labels.csv", "w") as csvfile:
        writer = csv.writer(csvfile, delimiter = ',')

        for i in y_label:
            writer.writerow(str(i))

    with open("./ResultsModelsLabels/RandomForest_predicted.csv", "w") as csvfile:
        writer = csv.writer(csvfile, delimiter = ',')
...
if __name__ == "__main__":
    ...
    train_random_forest(train_x, train_y)
    e_time = datetime.datetime.now()

    y_label, y_predicted, acc = test_random_forest(test_x, test_y)
